...
impl FrameThrottle for TimerBasedFrameThrottle {
    fn request_throttled_redraw(&self, winit_window: &winit::window::Window) {
        ...
        // DD-GUI patch: Windows reports 0 or 1 Hz for "the hardware's default rate" (some VMs
        // and remote sessions), which divided by zero below or gave 1 fps. Treat anything
        // under 10 Hz as unknown.
        let refresh_interval_millihertz = winit_window
            .current_monitor()
            .and_then(|monitor| monitor.refresh_rate_millihertz())
            .filter(|&millihertz| millihertz >= 10_000)
